<script lang="ts">
	...
	import type { ImageEngineConfig } from '$lib/apis/images';
	...
	export let config: ImageEngineConfig;
	...
	const paramsDescription = (engine: ImageEngineConfig['engine']) => {
		if (engine === 'gemini') {
			return $i18n.t(
	...
	const paramsPlaceholder = (engine: ImageEngineConfig['engine']) => {
		if (engine === 'gemini') {
	...
	const modelPlaceholder = (engine: ImageEngineConfig['engine']) => {
	...
	const baseURLPlaceholder = (engine: ImageEngineConfig['engine']) => {
		if (engine === 'gemini') {
