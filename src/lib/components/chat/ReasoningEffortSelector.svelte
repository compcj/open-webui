<script lang="ts">
	...
	$: selectedLabel = items.find((item) => item.value === value)?.label ?? $i18n.t('Default');
</script>
...
<Tooltip content={label ? `${label}: ${$i18n.t('Reasoning Effort')}` : $i18n.t('Reasoning Effort')}>
	<div class="flex min-w-0 items-center">
		<Select
			...
			triggerClass="flex min-w-0 max-w-[6.5rem] items-center gap-1 rounded-lg pl-2 pr-1.5 py-1 text-[0.8125rem] font-normal text-gray-600 transition-colors duration-100 hover:bg-gray-50/40 hover:text-gray-700 dark:text-gray-300 dark:hover:bg-gray-800/40 dark:hover:text-gray-200"
